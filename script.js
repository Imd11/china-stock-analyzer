...
async function callOpenRouterAPI(prompt) {
    ...
    try {
        ...
        console.log('message对象:', data.choices[0]?.message);
        console.log('message的所有属性:', Object.keys(data.choices[0]?.message || {}));
        
        if (!data.choices || !data.choices[0] || !data.choices[0].message) {
            console.error('API 响应格式异常:', data);
            throw new Error('API响应格式异常，缺少必要字段');
        }
        
        // GPT-4o使用标准的content字段
        const message = data.choices[0].message;
        ...
        console.log('最终使用的内容长度:', content ? content.length : 0);
        console.log('最终内容预览:', content ? content.substring(0, 200) + '...' : '无内容');
        
        if (!content || content.trim() === '') {
            console.error('API返回的内容为空，完整message对象:', message);
